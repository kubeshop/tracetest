...
func TestTransactionRunner(t *testing.T) {
	...
	t.Run("NoErrors", func(t *testing.T) {
		runTransactionRunnerTest(t, false, func(t *testing.T, actual transactions.TransactionRun) {
			assert.Equal(t, transactions.TransactionRunStateFinished, actual.State)
	...
	t.Run("WithErrors", func(t *testing.T) {
		runTransactionRunnerTest(t, true, func(t *testing.T, actual transactions.TransactionRun) {
			assert.Equal(t, transactions.TransactionRunStateFailed, actual.State)
...
func runTransactionRunnerTest(t *testing.T, withErrors bool, assert func(t *testing.T, actual transactions.TransactionRun)) {
	...
	transactionsRepo := transactions.NewTransactionsRepository(rawDB, testsRepo)
	transaction, err := transactionsRepo.Create(ctx, transactions.Transaction{
		Name:    "transaction",
	...
	done := make(chan transactions.TransactionRun, 1)
	sf := subscription.NewSubscriberFunction(func(m subscription.Message) error {
		tr := m.Content.(transactions.TransactionRun)
		if tr.State.IsFinal() {
			done <- tr
		}
	...
	var finalRun transactions.TransactionRun
	select {
	case finalRun = <-done:
