...
func TestTestRunnerEntities(t *testing.T) {
	t.Run("should fail the gates result because on of the required gates failed", func(t *testing.T) {
		gates := testrunner.RequiredGates{
		...
		assert.Equal(t, testrunner.RequiredGates{testrunner.RequiredGateAnalyzerRules, testrunner.RequiredGateTestSpecs}, result.Failed)
	...
	t.Run("should pass the gates result even if a non required gate fails", func(t *testing.T) {
		...
		assert.Equal(t, gates, result.Required)
		assert.Equal(t, testrunner.RequiredGates{testrunner.RequiredGateAnalyzerRules}, result.Failed)
	...
	t.Run("should handle failing the same gate twice", func(t *testing.T) {
		...
		assert.Equal(t, gates, result.Required)
		assert.Equal(t, testrunner.RequiredGates{testrunner.RequiredGateAnalyzerScore}, result.Failed)
