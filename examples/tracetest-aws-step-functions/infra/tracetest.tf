...
resource "aws_ecs_service" "tracetest_service" {
  name            = "${local.name}-service"
  cluster         = aws_ecs_cluster.tracetest_cluster.id
  task_definition = aws_ecs_task_definition.tracetest.arn
  desired_count   = 1
  launch_type     = "FARGATE"

  load_balancer {
    target_group_arn = aws_lb_target_group.tracetest_tg.arn
...
resource "aws_lb_target_group" "tracetest_tg" {
  name        = "${local.name}-tg"
  port        = 11633
  protocol    = "HTTP"
  vpc_id      = module.network.vpc_id
...
resource "aws_lb_listener" "tracetest_alb_listener" {
  load_balancer_arn = aws_lb.tracetest_alb.arn
  port              = "11633"
  protocol          = "HTTP"
