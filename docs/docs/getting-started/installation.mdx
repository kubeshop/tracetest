...
Set the `<namespace>` to your desired value, or omit it to install in the `default` namespace.

The `<release-name>` is set to `agent` to simplify the getting started guide.
...
helm install agent -n <namespace> tracetestcloud/tracetest-agent \\
...
helm install -n <namespace> -f values.yaml --create-namespace pokeshop .
```
